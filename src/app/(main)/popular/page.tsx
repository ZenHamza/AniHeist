...
export default function PopularPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/cached/popular?page=${page}&per_page=${perPage}`);
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-8 space-y-8">
      <div>
        ...
        <p className="text-text-secondary mt-1 text-sm">Highest-rated and most-watched of all time <span className="text-white/20">· Page {page}</span></p>
      </div>

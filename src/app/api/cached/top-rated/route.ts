...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `top-rated_p${page}_pp${perPage}`;

  const cached = cacheGet<{ data: Record<string, unknown>[]; meta: { count: number } }>(cacheKey, 30 * 24 * 60 * 60 * 1000);
  if (cached) {
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/api/top-rated?page=${page}&per_page=${perPage}`, {
